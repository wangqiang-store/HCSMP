        ...
        <el-col :span="24">
          ...
          >
            ...
            >
              ...
              <!-- 重伤人数 -->
              <template #seriousInjuriesToll>
                <el-input
                  type="text"
                  v-model="formData.seriousInjuriesToll"
                  autocomplete="off"
                >
                  <template #append>人</template>
                </el-input>
...
import {
  getAccidentInvestigationPage,
  createAccidentInvestigationInfo,
  deleteAccidentInvestigationInfo,
  updateAccidentInvestigationInfo,
} from "@/api/enforcement/investigation";
  ...
  setup(props) {
    ...
    let DATA: any = reactive({
      ...
      renderTable: [
...

        {
          ...
          prop: "happenTime",
          formatter: (row: any) => {
            return formatDate(row.happenTime);
          },
        },
        {
          label: "事发等级",
      ...
      rules: {
        ...
        deathToll: [{ pattern: verifyPlus, message: "人数格式是正整数或0" }],
        seriousInjuriesToll: [
          { pattern: verifyPlus, message: "人数格式是正整数或0" },
        ],
      },
      options: {
    ...
    let METHODS = reactive({
      ...
      handlePagingData: async (params: any) => {
        ...
        let { data } = await getAccidentInvestigationPage(params);
      ...
      handleConfirm: () => {
        ...
        proxy.$refs["formRef"].submitForm().then(async (valid: boolean) => {
          if (valid) {
            ...
            if (DATA.formData.id) {
              let { data } = await updateAccidentInvestigationInfo(
                DATA.formData
              );
              if (data.code === RES_SUCCESS) {
              ...
              let { data } = await createAccidentInvestigationInfo(
                DATA.formData
              );
              if (data.code === RES_SUCCESS) {
      ...
      handleBtnEvent: async ({ row, val }: any) => {
        ...
        switch (val.handleType) {
          ...
          case "delete":
            proxy
              .$confirm("是否确定删除该事故调查?", "提示", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
                type: "warning",
              })
              .then(async () => {
                let { data } = await deleteAccidentInvestigationInfo({
                  id: row.id,
                });
                if (data.code === RES_SUCCESS) {
                  proxy.$message({
