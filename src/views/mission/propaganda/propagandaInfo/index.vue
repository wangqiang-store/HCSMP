...
import {
  createPropagandaInfo,
  updatePropagandaInfo,
} from "@/api/mission/propaganda";
  ...
  setup(props) {
    ...
    let METHODS = reactive({
        ...
        formRef.value.submitForm().then(async (valid: boolean) => {
          if (valid) {
            ...
            if (DATA.formData.id) {
              ...
              let { data } = await updatePropagandaInfo(DATA.formData);
              ...
              let { data } = await createPropagandaInfo(DATA.formData);
              if (data.code === RES_SUCCESS) {
                ElMessage({
                  message: "添加成功",
                  type: "success",
      ...
      handleUploadSuccess: (res: any) => {
        ...
        console.log("成功回调", res, DATA.files);
      },
      handleRemove: (res: any, fileList: any) => {
        ...
        console.log("删除", res, fileList, DATA.files);
      },
    ...
    onMounted(() => {
      ...
      // 获取数据
      DATA.formData = Object.assign({}, STORE.state.globalData.propagandaInfo);
      DATA.fileList = [];
      DATA.files = [];
      DATA.formData.fileIds = [];
