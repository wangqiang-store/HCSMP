...
import { getStaffExpireByPage, setstaffMark, getTown } from '@/api/ent/expire';
  ...
  setup(props) {
    ...
    let DATA: any = reactive({
      ...
      renderTable: [
        ...
        },
        {
          ...
          prop: 'expirationTime',
          formatter: (row: any) => {
            return formatDate(row.expirationTime);
          },
        },
        {
          label: '人员名称',
    ...
    let METHODS = reactive({
      ...
      handleGetPagingData: async (pagingParams: any) => {
        ...
        let { data } = await getStaffExpireByPage(pagingParams);
        if (data.code === RES_SUCCESS) {
          ...
          DATA.tableData = newData.map((item: any) => {
            ...
            switch (METHODS.compareTime(item.expirationTime)) {
              case true:
                item.bg = true;
                break;
              case false:
