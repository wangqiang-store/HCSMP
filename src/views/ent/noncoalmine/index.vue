<template>
  <el-row class="container">
    ...
    <el-col
      :span="24"
      v-else
      :style="[1,2].includes(STORE.state.user.personnel.companyTypeId)"
    >
      <CompanyDetails v-if="[1,2].includes(STORE.state.user.personnel.companyTypeId)" />
...
import CompanyDetails from '@/views/ent/companyDetails/index.vue';
import {
  currentChange,
  getByKey,
  RES_SUCCESS,
...
export default defineComponent({
  ...
  setup(props) {
    ...
    let METHODS = reactive({
      ...
      handleCreate: async () => {
        await STORE.commit('setCompanyInfo', { type: 1, companyId: null });
        ROUTER.push({
          name: 'createCompany',
          params: {
