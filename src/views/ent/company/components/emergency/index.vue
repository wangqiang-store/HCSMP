  ...
  setup(props) {
    ...
    let METHODS = reactive({
      ...
      handleCancel: () => {
        ...
        proxy.$refs["dialogFormRef"].resetForm();
      },
      ...
      handleConfirm: () => {
        dialogFormRef.value.submitForm().then(async (valid: boolean) => {
          if (valid) {
            ...
            if(DATA.dialogFormData.fileIds[0]==","){
              DATA.dialogFormData.fileIds.splice(0,1)
            }
            if (DATA.dialogType) {
      ...
      uploadRemove: (res: any, fileList: any) => {
        let deleteUrl = "";
        if (res.response) {
        ...
        let fileIds = [];
        // 获取删除文件在文件列表对应的索引值
        let i = DATA.files.findIndex((item: any) => item.path == deleteUrl || item.url==deleteUrl);
        DATA.files.splice(i, 1);
        for (let item of DATA.files) {
          fileIds.push(item.id);
        }
        DATA.dialogFormData.fileIds = fileIds.join(",");
      },
    });
    watch(
      () => props.formData,
