<template>
  ...
  >
    <div style="margin-right: 10px; display: flex; flex-wrap: wrap">
      ...
      >您的浏览器不支持视频播放</video> -->
      <template
        v-if="videoList.length > 0 && videoFlag == false && refreshVideo"
      >
        <div
    ...
    >
      <i class="el-icon-plus"></i>
      <template #tip>
        <div class="el-upload__tip pt-10" v-if="tip">{{ tip }}</div>
      </template>
    </el-upload>
  </div>
</template>
  ...
  setup(props, { emit }) {
    ...
    let METHODS: any = reactive({
      handleRemove(file: any, fileList: any) {
        ...
        // DATA.fileList = [];
        emit("uploadRemove", file, fileList);
      },
    ...
    watch(
      () => props.videoList,
      (newValue) => {
        DATA.videoLists = props.videoList;
